...
ACCELERATION_RATE = 6/3.6 # Acceleration rate when speed to low
...
class Car:
  ...
  def move_ramp(self,simulation, time_interval):
    if self.position < 250:
      ...
      self.y+=self.speed * time_interval/3 # divided by 3 is for angle
...
class Simulation:
  ...
  def changed_destination(self, car):
      ...
      if cars_lane_slower and car.position < 650:
        if random.random() <= 0.025:  # probability to change destination if traffic too slow and fast in other lane
          car.need_to_change_destination = True
  ...
  def late_merge(self,car, time_interval):
    ...
    if car.on_ramp:
      ...
      late_position=car.position>=460 or car.speed < 3
      if late_position:
        if not car.merge:
          car.speed = 0
        ...
        for lane_car in target_lane:
          if car.position-240 < lane_car.position < car.position -10:
            lane_car.speed = max(lane_car.speed - DECELERATION_RATE, 0)
        # Checking zone if they can merge or to slow down itself to keep up with the car in front
        for lane_car in target_lane:
